...
100 poke s+24,15
105 rem set pulse width (12 bits) for voice one pulse modulation
106 poke s+3,8 :rem set high byte
108 poke s+2,0 :rem set low byte
...
150 poke s+1,hf
160 poke s,lf
170 rem set waveform as pulse (bit 6) and activate gate bit (bit 0)
...
220 for t = 1 to dr
230 next
240 rem set waveform as pulse (bit 6) and deactivate gate bit (bit 0)
