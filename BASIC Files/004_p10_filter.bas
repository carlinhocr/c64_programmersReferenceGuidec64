...
70 poke s+5,9
80 poke s+6,0
85 rem set low frequency for filter cut off
88 poke s+22,128
89 rem set high frequency for filter cut off
94 poke s+21,0
96 rem activate filter for voice 1
98 poke s+23,1
99 rem set volume to maxium and high pass filter
100 poke s+24,79
...
150 poke s+1,hf
160 poke s,lf
170 rem set waveform as sawtooth (bit 5) and activate gate bit (bit 0)
...
220 for t = 1 to dr
230 next
240 rem set waveform as sawtooth (bit 5) and deactivate gate bit (bit 0)
